...
actor class DRC721(_name : Text, _symbol : Text) {
    ...
    public shared ({caller}) func getCallerTokens() : async [TokenWithURI]{
        
        var registry:List<TokenWithURI> = List.nil<TokenWithURI>();
        if(Principal.isAnonymous(caller)){
            return List.toArray(registry);
        }; 
        for ((k,v) in owners.entries()) {
            if(v == caller){
                let tokenUri:?Text = _tokenURI(k);
                let token:TokenWithURI = {
                    tokenId=k;
                    imageUrl=tokenUri;
                };
                registry := List.push<TokenWithURI>(token,registry);
            }
        };
        return List.toArray(registry);
    };
